...
def randomint(l: int):
    return "".join(random.choice(string.digits) for x in range(l))

def randomstr(l: int):
    return "".join(random.choice(string.ascii_letters) for x in range(l))

def get_cookies():
    r = requests.get("https://www.czc.cz/")
    cookies = r.headers["set-cookie"].split(", ")
    #print(cookies)
    token = ""
    session_id = ""
    for x in cookies:
        if x.startswith("XSRF-TOKEN="):
            token = x
    return {"cookies": cookies, "token": token}

def register():
    cookies = get_cookies()
    token = cookies["token"].split(";")[0].replace("XSRF-TOKEN=", "")
    print(cookies)
    print(token)
    r = requests.post("https://www.czc.cz/registrace/nova",
        headers = {
        "cookies": f"{cookies['cookies']}",
        ...
        "referer": "https://www.czc.cz/registrace/nova",
        ...
        "email": f"{mail}+{random.randint(5,600)}@{domain}",
        "phoneNumberFormatted": "+420602"+randomint(6),
        "name": randomstr(5),
        "surname": randomstr(7),
        "password": randomstr(15),
        "firmInformation": False,
        "companyNo": "",
        "taxNo": "",
        "companyName": "",
        "isicCommand.present": False,
        "isicCommand.isicCardholderName": "",
        "isicCommand.isicCardNumber": "",
        "street": "Československé armády 217",
        "city": "Slavkov u Brna",
        "zipCodeFormatted": 68401,
        "fullBankAccountNo": "",
        "marketingAgreement": True,
        "countryId": "CN.0-AAAAAAAAAAAAAAAAAAAAA"      
        })
    print(r.status_code)
    print("finished")
